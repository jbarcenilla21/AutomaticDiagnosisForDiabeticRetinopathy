...
def build_train_transforms(
    ...
    All magnitude-based augmentations are scaled by ``intensity`` (0.0–1.0):
 
      ┌────────────────────────┬──────────────────────────────────────┐
      │ Transform              │ Magnitude at intensity=1.0           │
      ├────────────────────────┼──────────────────────────────────────┤
      │ RandomRotation         │ ±30°                                 │
      │ RandomVFlip (prob)     │ p = 0.3                              │
      │ ColorJitter brightness │ 0.50                                 │
      │ ColorJitter contrast   │ 0.50                                 │
      │ ColorJitter saturation │ 0.30                                 │
      │ ColorJitter hue        │ 0.10                                 │
      │ GaussianNoise std      │ 0.03                                 │
      └────────────────────────┴──────────────────────────────────────┘
 
    At intensity=0.0 only deterministic crops and (optionally) CLAHE remain.
 
    Args:
        img_size:     Final crop size (height == width).
        rescale_size: Intermediate resize target before cropping.
        rigid:        Enable rotation and vertical flip.
        regularize:   Enable Gaussian noise.
        clahe:        Enable CLAHE green-channel enhancement.
        intensity:    Augmentation magnitude scalar in [0.0, 1.0].
    """
    ...
    if rigid:
        steps += [
            RandomHFlip(p=0.5),                           # always 50% when rigid=True
...
def get_dataloader(
    ...
    Args:
        ...
        aug_intensity:          Passed to build_train_transforms when transform=None.
    ...
    if transform is None:
        transform = build_train_transforms(intensity=aug_intensity) if is_train else build_eval_transforms()
